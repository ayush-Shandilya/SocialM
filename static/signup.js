document.addEventListener("DOMContentLoaded", () => {
    let test = [false,false,false,false,false,false];
    document.querySelectorAll(".inp").forEach(input => {
        input.addEventListener('input', () => {
            ...
            if(input.classList.contains('usrnm')){
                if(input.value.trim().length !== 0) {
                    test[3]=true;
                }

            }
            if(input.classList.contains('pswrd')){
                document.querySelector('.cpswd').value = "";
                document.querySelector('.cpswd').parentElement.querySelector('span').innerText = "";
                test[4] = false;
                if(input.value.trim().length !== 0) {test[3]=true;}
                else{
                    test[3] = false;
                }
            }
            if(input.classList.contains('cpswd')){
                if(input.value.trim().length !== 0) {
                    if(input.value !== document.querySelector('.pswrd').value) {
